...
This is an advanced developer section that describes creation of a new release for cellmaps_annotate_hierarchy and deploying
...
Be sure to update the version in ``cellmaps_annotate_hierarchy/__init__.py`` Omit the ``v`` when setting the version in the file.

Commit the above changes and push to **main**:

.. code-block::

    git commit -m 'message...' cellmaps_annotate_hierarchy/__init__.py
...
From cellmaps_annotate_hierarchy source tree generate the wheel and tar.gz files
...
.. code-block::
    ...
    $ tree
    dist
    |-- cellmaps_annotate_hierarchy-0.1.0-py2.py3-none-any.whl
    `-- cellmaps_annotate_hierarchy-0.1.0.tar.gz
...
#. From https://github.com/idekerlab/cellmaps_annotate_hierarchy click on releases link.
...
Once the above is done verify deploy was successful by browsing to https://testpypi.python.org/pypi/cellmaps_annotate_hierarchy and verifying new release was deployed.

Also test the package by installing it locally by running this:

.. code-block::

    pip install -i https://testpypi.python.org/pypi cellmaps_annotate_hierarchy
...
.. code-block::
    ...
    pip install cellmaps_annotate_hierarchy


.. _Travis: https://travis-ci.org/idekerlab/cellmaps_annotate_hierarchy
.. _main: https://github.com/idekerlab/cellmaps_annotate_hierarchy/tree/main
