...
import cellmaps_annotate_hierarchy
...
def _parse_arguments(desc, args):
    ...
    parser.add_argument('--version', action='version',
                        version=('%(prog)s ' +
                                 cellmaps_annotate_hierarchy.__version__))
...
def main(args):
    ...
    :return: return value of :py:meth:`cellmaps_annotate_hierarchy.runner.CellmapsannotatehierarchyRunner.run`
    ...
    Invokes run() method on CellmapsannotatehierarchyRunner

    """.format(version=cellmaps_annotate_hierarchy.__version__)
    theargs = _parse_arguments(desc, args[1:])
    theargs.program = args[0]
    theargs.version = cellmaps_annotate_hierarchy.__version__

    try:
        _setup_logging(theargs)
        return CellmapsannotatehierarchyRunner(theargs.exitcode).run()
